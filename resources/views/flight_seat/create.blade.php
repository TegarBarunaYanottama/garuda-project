...
<head>
    ...
    <title>Tambah Kursi Baru - Garuda Indonesia</title>
    ...
    <div class="container">
        ...
        <form method="POST" action="{{ route('flight_seat.store') }}">
            ...
            <div class="form-group">
                ...
                <input type="text" id="flight_number" name="flight_number" value="{{ old('flight_number', 'GA123') }}" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="seat_row" name="seat_row" required>
                    ...
                    <option value="A" {{ old('seat_row') == 'A' ? 'selected' : '' }}>A</option>
                    <option value="B" {{ old('seat_row') == 'B' ? 'selected' : '' }}>B</option>
                    <option value="C" {{ old('seat_row') == 'C' ? 'selected' : '' }}>C</option>
                    <option value="D" {{ old('seat_row') == 'D' ? 'selected' : '' }}>D</option>
                    <option value="E" {{ old('seat_row') == 'E' ? 'selected' : '' }}>E</option>
                    <option value="F" {{ old('seat_row') == 'F' ? 'selected' : '' }}>F</option>
                </select>
            </div>

            <div class="form-group">
                <label for="seat_number">Nomor Kursi</label>
                <input type="number" id="seat_number" name="seat_number" min="1" max="10" value="{{ old('seat_number', 1) }}" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="status" name="status" required>
                    ...
                    <option value="booked" {{ old('status') == 'booked' ? 'selected' : '' }}>Terisi</option>
                </select>
            </div>

            <div class="form-group">
                <label for="passenger_name">Nama Penumpang (Opsional)</label>
                <input type="text" id="passenger_name" name="passenger_name" value="{{ old('passenger_name') }}">
            </div>

            <div class="form-group">
                <label for="passenger_email">Email Penumpang (Opsional)</label>
                <input type="email" id="passenger_email" name="passenger_email" value="{{ old('passenger_email') }}">
            </div>

            <button type="submit">Simpan Kursi</button>
            <a href="{{ route('flight_seat.index') }}" class="btn-back">Kembali ke Daftar Kursi</a>
        </form>
    </div>
</body>
</html>